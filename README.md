...
Demo-App :8081
  |
  | erzeugt authId und redirectet mit authId
  v
Auth Manager :8082
  |
  | übernimmt authId unverändert
...
Auth Manager
  |
  | Access Token + Refresh Token serverseitig unter authId
  |
...
Wichtig: Die `authId` wird **ausschließlich von der Demo-App erzeugt**. Der Auth Manager erzeugt keine eigene `authId`; er nimmt den Wert entgegen und verwendet ihn nur als opaque Schlüssel für die zugehörigen Tokens.
...
3. Der Auth Manager übernimmt genau diese `authId`, erzeugt mit Nimbus `state` und `nonce` und leitet den Browser zu Keycloak weiter.
...
6. Der Auth Manager validiert das ID Token und speichert die Tokens serverseitig unter der von der App gelieferten `authId`.
...
- lokale Session
- Erzeugen und Besitz der `authId`
...
Ebenfalls reines Java 8 mit Nimbus `oauth2-oidc-sdk` im `jdk8`-Build. Der Maven-Compiler steht auf Source/Target 8 und das Docker-Image verwendet Temurin 8 für Build und Runtime.

Verantwortlich für:

- Entgegennahme der von der App erzeugten `authId`
- Erzeugen der Keycloak-Login-URL
- `state` und `nonce`
- Callback-Verarbeitung
- Authorization-Code-Exchange
- ID-Token-Validierung
- Speicherung von Access- und Refresh-Token unter der gelieferten `authId`
- automatischen und erzwungenen Refresh
- Bereitstellung eines gültigen Access Tokens für die App

Der Auth Manager **erzeugt niemals selbst eine `authId`**.
...
- `authId` ist im Demo ein zufälliger opaque Identifier, der von der App erzeugt wird
